...
contract TestAccounting is Test {
    ...
    /**
     * - Long balance should always equal longTokenBalance + longAccumulatedFees + longCollateral
     * - Short balance should always equal shortTokenBalance + shortAccumulatedFees + shortCollateral
     */
    uint256 constant TOLERANCE = 1e6; // 1 USDC or 0.000001 ETH tolerance

    /// @dev - Test that accounting is always correct and that all money in / out the system is accounted for
    function invariant_test_accounting() public {
        uint256 longBalance = handler.getLongBalance();
        uint256 shortBalance = handler.getShortBalance();

        uint256 expectedLongBalance = handler.getLongTokenBalance() + handler.getLongAccumulatedFees()
            + handler.getLongCollateral() + handler.getLongPnlPaidOut();

        uint256 expectedShortBalance = handler.getShortTokenBalance() + handler.getShortAccumulatedFees()
            + handler.getShortCollateral() + handler.getShortPnlPaidOut();

        console2.log(handler.numberOfIncreases(), "Number of increase trades");
        console2.log(handler.numberOfDecreases(), "Number of decrease trades");

        assertEq(longBalance, expectedLongBalance, "Long balance mismatch");
        assertEq(shortBalance, expectedShortBalance, "Short balance mismatch");
    }
}
