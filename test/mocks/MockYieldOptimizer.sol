...
import {SafeERC20} from "src/tokens/SafeERC20.sol";
...
import {IRewardTracker} from "src/rewards/interfaces/IRewardTracker.sol";
...
/**
 * @title MockYieldOptimizer
 */
contract MockYieldOptimizer is ERC20, OwnableRoles, ReentrancyGuard {
    ...
    error YieldOptimizer_AllFailed();
    ...
    // @audit - Need to make sure it can only be called after all deposits attempted
    // @audit - Need to make sure users are refunded if any deposits fail
    function executeDeposit(bytes32 _openRequestKey) external nonReentrant {
    ...
    // @audit - Need to make sure it can only be called after all deposits attempted
    function executeWithdrawal(bytes32 _openRequestKey) external nonReentrant {
    ...
    function _calculateMintAmount(uint256 _amount, address _token) private view returns (uint256) {
        ...
        console2.log("Lp token price: ", lpTokenPrice);

        // Get the price of the token being transferred in
        uint256 tokenInPrice = _token == usdc ? 1e30 : 2500e30;
        ...
        console2.log("Token value in USD: ", tokenValueUsd);
